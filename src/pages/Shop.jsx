...
const Shop = () => {
  ...
  const filteredProducts = PRODUCTS.filter(product => {
    ...
    return 0; // featured/newest defaults to original order for hardcoded
  });

  const handleCollectionChange = (colName) => {
    setSearchParams(colName === 'All' ? {} : { collection: colName });
        ...
        <main className="flex-1 py-12 lg:pl-12">
          {filteredProducts.length > 0 ? (
            ...
            <div className="py-32 flex flex-col items-center text-center">
              ...
              <button onClick={clearFilters} className="btn btn-primary px-10">Clear All Filters</button>
            </div>
          )}
        </main>
      </div>
