...
interface ProductClientProps {
  ...
  /** Full catalog, used for the "related products" rail. */
  initialProducts: Product[];
}

const ProductDetail: React.FC<ProductClientProps> = ({ initialProduct, initialProducts }) => {
  const params = useParams();
  // The route segment is named [slug] but historically carried the numeric
  // product id, and every internal link still uses one of the two. Accept
  // slug, numeric id, or Sanity _id so old links, shared links and the
  // slug-based sitemap all resolve to the same page.
  const id = params?.slug as string;
  const { addToCart } = useCart();
  const { user } = useAuth();
  
  // Both seeded from the server render: the gallery, price and related rail are
  // in the first HTML response, so images start downloading during parse rather
  // than after hydration fetches the whole catalog.
  ...
  useEffect(() => {
    // Fallback only. Runs when the server could not resolve the product (a
    // failed Sanity read), so a reload still has a chance of rendering.
    ...
    const loadProduct = async () => {
      try {
        ...
        const found = fetched.find(
          p => p.slug === id || String(p.id) === id || p._id === id
        );
        if (found) {
          setProduct(found);
        }
  ...
  // Index-aligned base64 previews so a gallery slide is never an empty box.
  const displayLqip: string[] = (activeVariant && activeVariant.images.length > 0)
    ? (activeVariant.lqip || [])
    : (product.lqip || []);

  const galleryImage = (image: unknown, idx: number, main: boolean) => ({
    ...imageProps(image, {
      widths: main ? DETAIL_WIDTHS : [96, 160, 240],
      sizes: main ? '(max-width: 1023px) 100vw, 620px' : '160px',
      quality: main ? 82 : 70,
      fallbackWidth: main ? 800 : 160,
    }),
    // The first slide is the largest thing on the page; everything after it can
    // wait until the customer scrolls or swipes to it.
    loading: (main && idx === 0 ? 'eager' : 'lazy') as 'eager' | 'lazy',
    decoding: (main && idx === 0 ? 'sync' : 'async') as 'sync' | 'async',
    fetchPriority: (main && idx === 0 ? 'high' : 'auto') as 'high' | 'auto',
  });
              ...
              >
                {displayImages.map((img: any, idx: number) => (
                  <div
                    ...
                    style={displayLqip[idx] ? {
                      backgroundImage: `url(${displayLqip[idx]})`,
              ...
              <div>
                ...
                <div className="flex items-center w-32 border border-neutral-200">
                  ...
                  <button 
                    ...
                    className="flex-1 py-3 hover:bg-neutral-50 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                  >
              ...
              <div className="bg-neutral-50 p-8 rounded-2xl border border-neutral-100/50">
                ...
                {user ? (
                  ...
                  <div className="text-center py-4 space-y-4">
                    ...
                    >
                      <Link href={`/login?redirect=/product/${product.slug || product.id}`}>
                        Sign In to Review
                      </Link>
                    </Button>
                  </div>
