...
interface ShopClientProps {
  ...
  /** Resolved server-side from ?collection= so the first render needs no client state. */
...
const ShopContent: React.FC<ShopClientProps> = ({
  ...
  // Seeded from the server render, so the grid and its images are already in
  // the initial HTML rather than appearing after a client-side round trip.
  ...
  useEffect(() => {
    // Fallback only: with a healthy server render this never runs. It covers the
    // case where the Sanity read failed during SSR. Note the two requests now go
    // out together — they used to be awaited one after the other.
    ...
    const loadData = async () => {
      setLoading(true);
      try {
        const [fetchProds, fetchCats] = await Promise.all([getProducts(), getCategories()]);
        if (cancelled) return;
              ...
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3 gap-x-4 lg:gap-x-8 gap-y-8 lg:gap-y-16">
                <AnimatePresence mode="popLayout">
                  {filteredProducts.map((product, idx) => (
                    ...
                    >
                      {/* The first two rows of the grid are above the fold on
                          most viewports, so those load eagerly. */}
...
const ShopClient: React.FC<ShopClientProps> = (props) => {
  return (
